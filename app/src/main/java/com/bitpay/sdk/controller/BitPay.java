...
public class BitPay {
     ...
     */
    public BitPay(String clientName, String envUrl, Context c) throws BitPayException {
    ...
    public BitPay(String clientName, Context c) throws BitPayException {
        this(clientName, BITPAY_URL, c);
    }

    public BitPay(Token token, String url, Context c) throws BitPayException {
        this("Android Authorized Client", url, c);

        _tokenCache.put(token.getFacade(), token.getValue());
    }

    public BitPay(Context c) throws BitPayException {
        this(BITPAY_PLUGIN_INFO, BITPAY_URL, c);
    }
     ...
     * @throws BitPayException
     */
    public BitPay(ECKey ecKey, String clientName, String envUrl, Context c) throws BitPayException {
        _ecKey = ecKey;
        _context = c;
        this.deriveIdentity();
        _clientName = clientName;
    ...
    public String requestClientAuthorization(String facade) throws BitPayException {
        ...
        pairCode = tokens.get(0).getPairingCode();
    ...
    public Invoice createInvoice(Invoice invoice, String facade) throws BitPayException {
        ...
        try {
            ...
            String data = json_res.getJSONObject("data").toString();
            invoice = mapper.readerForUpdating(invoice).readValue(data);
        } catch (JsonProcessingException e) {
    ...
    public List<Invoice> getInvoices(String dateStart, String dateEnd) throws BitPayException {
        ...
        String responseString = this.responseToJsonString(response);
        List<Invoice> invoices;
        try {
            invoices = mapper.readValue(responseString, InvoicesWrapper.class).data;
        } catch (JsonProcessingException e) {
    ...
    {
        if (KeyUtils.privateKeyExists(_context))
        {
            _ecKey = KeyUtils.loadEcKey(_context);
        ...
        else
        {
            ...
            KeyUtils.saveEcKey(_ecKey, _context);
        }
    }
    ...
    private void adaptTokens(Hashtable<String, String> tokenCache) {
        Hashtable<String, String> newTokens = new Hashtable<>();
        for(Map.Entry<String, String> s : tokenCache.entrySet()) {
            if(FACADE_MERCHANT.equals(s.getKey())) {
                newTokens.put(FACADE_POS, s.getValue());
            }
        }
        tokenCache.putAll(newTokens);
    }
	...
	private String responseToJsonString(HttpResponse response) throws BitPayException 
	{
		...
		try {
		    ...
		    if (node != null)
		    {
		    	String message = "Multiple errors: {" + node.toString() + "}";
		    	if (node.isArray()) {
